...
// --- KATILMA İŞLEMİ (GÜNCELLENDİ: ARTIK VERİTABANINA KAYDEDİYOR) ---
$mesaj = "";
$mesajTuru = ""; // Başarılı mı hatalı mı?
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['katil_btn'])) {
    ...
    $kayitSql = "INSERT INTO katilimcilar (etkinlik_adi, ad_soyad, okul, ogr_no) VALUES (:etkinlik, :ad, :okul, :no)";
    $kayitStmt = $conn->prepare($kayitSql);
    $sonuc = $kayitStmt->execute([
    ...
    if ($sonuc) {
        $mesaj = "Harika! <b>$isim</b>, <b>$etkinlikAdi</b> için kaydın başarıyla veritabanına işlendi. ($okul) 🎉";
        $mesajTuru = "success";
    } else {
        $mesaj = "Bir hata oluştu, kayıt yapılamadı.";
        $mesajTuru = "danger";
    }
}
...
<head>
    ...
    <style>
        :root { --primary: #6366f1; --secondary: #8b5cf6; --glass: rgba(255, 255, 255, 0.7); --glass-border: rgba(255, 255, 255, 0.5); }
        body { font-family: 'Plus Jakarta Sans', sans-serif; background-color: #f3f4f6; color: #1f2937; overflow-x: hidden; min-height: 100vh; position: relative; }
        .blob-bg { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -1; overflow: hidden; background: #f0f2f5; }
        .blob { position: absolute; filter: blur(80px); opacity: 0.6; animation: moveBlob 20s infinite alternate; }
        ...
        .nav-floating { margin-top: 20px; background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(12px); border-radius: 50px; padding: 10px 30px; box-shadow: 0 10px 30px rgba(0,0,0,0.05); border: 1px solid var(--glass-border); }
        ...
        .hero-container { text-align: center; padding: 80px 0 60px 0; }
        .hero-title { font-size: 3.5rem; font-weight: 800; line-height: 1.1; margin-bottom: 15px; letter-spacing: -1px; color: #111827; }
        .hero-desc { font-size: 1.2rem; color: #6b7280; margin-bottom: 40px; font-weight: 500; }
        .search-glass { background: rgba(255, 255, 255, 0.6); backdrop-filter: blur(20px); border: 2px solid white; border-radius: 20px; padding: 10px; box-shadow: 0 20px 40px rgba(0,0,0,0.05); display: flex; max-width: 800px; margin: 0 auto; transition: 0.3s; }
        ...
        .btn-search-round { width: 50px; height: 50px; border-radius: 15px; background: #111827; color: white; border: none; display: flex; align-items: center; justify-content: center; transition: 0.3s; }
        .btn-search-round:hover { background: #4f46e5; transform: rotate(10deg); }
        .card-modern { background: rgba(255, 255, 255, 0.9); border-radius: 24px; border: 1px solid white; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.03); transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); height: 100%; display: flex; flex-direction: column; }
        .card-modern:hover { transform: translateY(-12px) scale(1.02); box-shadow: 0 20px 40px rgba(79, 70, 229, 0.15); }
        ...
        .date-float { position: absolute; top: 15px; right: 15px; background: rgba(255,255,255,0.95); padding: 8px 14px; border-radius: 12px; text-align: center; font-weight: 800; box-shadow: 0 5px 15px rgba(0,0,0,0.1); }
        .alku-float { position: absolute; top: 15px; left: 15px; background: #FFD700; color: #000; font-size: 0.75rem; font-weight: 800; padding: 6px 14px; border-radius: 30px; box-shadow: 0 5px 15px rgba(255, 215, 0, 0.4); }
        .card-content { padding: 25px; flex-grow: 1; display: flex; flex-direction: column; }
        .cat-pill { font-size: 0.7rem; font-weight: 700; text-transform: uppercase; color: #6366f1; background: #e0e7ff; padding: 5px 12px; border-radius: 20px; display: inline-block; margin-bottom: 10px; width: fit-content; }
        .card-h { font-weight: 800; font-size: 1.2rem; margin-bottom: 8px; color: #111827; }
        .timer-box { background: #f9fafb; border-radius: 12px; padding: 10px; font-size: 0.85rem; margin-top: auto; margin-bottom: 15px; display: flex; align-items: center; gap: 8px; }
        .btn-group-custom { display: flex; gap: 10px; }
        .btn-join-main { flex: 2; background: #111827; color: white; border: none; padding: 12px; border-radius: 12px; font-weight: 600; transition: 0.3s; }
        ...
        .btn-detail-main { flex: 1; background: white; border: 2px solid #f3f4f6; color: #374151; font-weight: 600; border-radius: 12px; text-decoration: none; display: flex; align-items: center; justify-content: center; transition: 0.3s; }
...
<body>
    ...
    <div class="container mt-3" style="position:relative; z-index:1000;">
        <div class="alert alert-<?php echo $mesajTuru; ?> text-center shadow-sm border-0 rounded-4">
                    ...
                    <div class="card-modern">
                        ...
                        <div class="card-content">
                            ...
                            <div class="btn-group-custom">
                                ...
                                <a href="detay.php?id=<?php echo $etkinlik['id']; ?>" class="btn-detail-main">İncele</a>
                            </div>
                        </div>
                    </div>
                </div>
